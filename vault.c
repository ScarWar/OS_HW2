...
short findGaps(int blockIndexes[], ssize_t blockSizes[], Gaps *gaps, ssize_t fileSize) {
    ...
    if (gaps == NULL) {
        ...
        REPORT_ERR_MSG("fileSize argument can't be negative");
        return -1;
    }
...
int writeToBlock(int vfd, int ifd, ssize_t dataToWrite, DataBlock dataBlock, off_t inputOffset) {
    ...
    if (lseek(vfd, dataBlock.dataBlockOffset, SEEK_SET) == -1) {
        REPORT_ERR;
        return 0;
    }
    if (lseek(ifd, inputOffset, SEEK_SET) == -1) {
        REPORT_ERR;
        return 0;
    }
...
long getSizeFromString(char *string) {
    ...
    if (sscanf(string, "%lld%c", &size, &type) != 2) {
...
ssize_t init(int vfd, long long dataSize) {
    ssize_t written;
    Vault *vault;
    ...
    if (lseek(vfd, dataSize - 1, SEEK_SET) == -1) {
        REPORT_ERR;
        return 0;
    }
    written = write(vfd, "#", 1);
...
int removeFile(Vault *vault, int vfd, int fileIndex) {
    ...
    for (i = 0; i < file.numberOfPartitons; ++i) {
        if (lseek(vfd, file.dataBlock[i].dataBlockOffset, SEEK_SET) == -1) {
            REPORT_ERR;
            return 0;
        }
        ...
        if (lseek(vfd, file.dataBlock[i].dataBlockSize - 16, SEEK_CUR) == -1) {
            REPORT_ERR;
            return 0;
        }
        written = write(vfd, ZERO_DELIMITER, 8);
...
int fetchFile(Vault *vault, int vfd, int fileIndex) {
    ...
    for (i = 0; i < file.numberOfPartitons; ++i) {
        if (lseek(vfd, file.dataBlock[i].dataBlockOffset + 8, SEEK_SET) == -1) {
            REPORT_ERR;
            return 0;
        }
...
int status(Vault *vault) {
    ...
    fragRatio = getFragmentationRation(vault);
    if (fragRatio < 0)
        return 0;
...
int defragmentation(Vault *vault, int vfd) {
    ...
    for (i = 0; i < numOfPartitions; ++i) {
        ...
        if (gapIndex != -1) {
            ...
            if (lseek(vfd, tupleArray[i].offset, SEEK_SET) == -1) {
                REPORT_ERR;
                return 0;
            }
            ...
            for (j = 0; j < file.numberOfPartitons; ++j)
                if (tupleArray[i].offset == file.dataBlock[j].dataBlockOffset)
                    break;
            if (lseek(vfd, file.dataBlock[j].dataBlockSize - 16, SEEK_CUR) == -1) {
                REPORT_ERR;
                return 0;
            }
            ...
            while (dataToWrite > 0) {
                ...
                if (lseek(vfd, readOffset, SEEK_SET) == -1) {
                    REPORT_ERR;
                    return 0;
                }
                ...
                if (lseek(vfd, writeOffset, SEEK_SET) == -1) {
                    REPORT_ERR;
                    return 0;
                }
...
int main(int argc, char **argv) {
    ...
    if (errorFlag != -1) {
        if (strcmp(opName, "init") == 0) {
            ...
            if (vfd < 0) {
                REPORT_ERR_MSG("Failed to open vault file");
                errorFlag = -1;
            } else {
            ...
            if (vfd < 0) {
                ...
                if (vault != NULL) {
                    ...
                    if (index == -1) {
                        REPORT_ERR_MSG("File does not exist in the vault");
                        errorFlag = -1;
